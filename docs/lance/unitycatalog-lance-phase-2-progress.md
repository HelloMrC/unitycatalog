...
**当前边界：** 已打通 UC 到 HTTP worker 的命令协议；read retry、write no-retry 细节、真实 Arrow streaming 转发和真实 worker E2E 仍待后续小步。
...
Worker HTTP backend 首批协议层已完成；剩余重点是 retry/streaming 边界和真实 worker E2E。
...
| W2-5: Worker HTTP backend | 15.6 | ⚠️ 部分 | Add Lance worker HTTP backend（WorkerHttpLanceExecutionBackend + fake worker HTTP path/header/error contract 完成；retry/streaming/real worker E2E 待续） |
...
| LancePhase2WorkerHttpBackendRestTest | 7 | Enabled | P2-WORKER-001/002/004/005/006/007/008 fake HTTP worker 合同覆盖 |
...
高中优先级阻塞项已清空；后续立即可做项以 Worker retry/streaming 边界为主。

### 8.2 需要测试 fixture

2. **Worker retry/streaming** - read retry、write no-retry、Arrow streaming 转发
...
| backend 未配置、backend 超时、worker 错误有稳定错误语义 | ⚠️ disabled backend + fake/HTTP worker timeout/error mapping 就绪，real worker retry 缺失 |
