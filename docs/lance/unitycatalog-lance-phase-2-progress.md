...
**当前边界：** 已打通 UC 到 HTTP worker 的命令协议，并覆盖 health、read retry / write no-retry、Arrow request body handoff 与 Arrow query response handoff 合同；UC 入口非聚合 streaming/backpressure 和真实 worker E2E 仍待后续小步。
...
Worker HTTP backend 协议层、health probe、retry/no-retry 与 Arrow request/response handoff 已完成；剩余重点是 UC 入口非聚合 streaming/backpressure 和真实 worker E2E。
...
| LancePhase2WorkerHttpBackendRestTest | 11 | Enabled | P2-WORKER-001~010 fake HTTP worker 合同覆盖，P2-WORKER-005/P2-ARROW-008 已验证 Arrow request/response handoff |
